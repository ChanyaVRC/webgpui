...
### 3.5.1 `webgpui-render-cuda`
- CUDA device and context initialization
- CUDA kernel compilation and management
- GPU memory allocation (textures, buffers)
- Frame rendering via CUDA kernels
- GPU timestamp query metrics
- Implements `webgpui-render` trait (same as wgpu backend)
- Optional backend: requires `cuda` feature flag
- Platform support: Linux (x86_64), Windows (x86_64) via CUDA Toolkit 11.8+
...
- `backend-wgpu`: enable `webgpui-render-wgpu` (requires wgpu-compatible GPU)
- `backend-cuda`: enable `webgpui-render-cuda` (requires NVIDIA GPU with CUDA Toolkit 11.8+)
  - Note: both backends can be enabled simultaneously for runtime selection
  - Runtime backend selection via `BackendSelector` enum in `webgpui-render`
  - Feature-gated binary: only one backend per executable, but libraries can include both
  - Application can query available backends: `BackendSelector::available()`
  - Application can check backend availability: `BackendSelector::Cuda.is_available()`
- `platform-winit`: enable `webgpui-platform-winit` (winit window system)
